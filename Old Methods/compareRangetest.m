...
%% Implementation
function compareRangetest(setting,tail,alpha,stepsize,nruns,filename)
% Setting = 'A', 'B', 'C'.
...
for i=1:nruns
    ...
    X_withnew = [X;xnew];
    rangeY=max(Y)-min(Y);
    ytrial2 = (min(Y)-2*rangeY):stepsize:(max(Y)+2*rangeY);
    ytrial1 = min(Y):stepsize:max(Y);
    ...
    % run method
    tic;
    [yconf1,modelsize1,sc1] = conformalLassoCtnFit(X,Y,xnew,alpha,ytrial1,lambda);
    t1=toc;time1=time1+t1;fprintf(2,'1');tic;
    [yconf2,modelsize2,sc2] = conformalLassoCtnFit(X,Y,xnew,alpha,ytrial2,lambda);
    t2=toc;time2=time2+t2;fprintf(2,'2');tic;
    [yconf3,modelsize3,sc3] = conformalLOO(X,Y,xnew,alpha,ytrial1,lambda);
    t3=toc;time3=time3+t3;fprintf(2,'3');tic;
    [yconf4,modelsize4,sc4] = conformalLOO(X,Y,xnew,alpha,ytrial2,lambda);
    ...
    % format print
    fprintf(fileID,'\t\t\t\tLassoCtnFit\tLassoCtnFit_wide\t\tLOO\t\t\t\tLOO_wide\n');
